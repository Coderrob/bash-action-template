...
record_event() {
    ...
    local event
    event="$(create_json_event "${category}" "${message}" "${metadata}")"
    EVENTS+=("${event}")
...
record_warning() {
    ...
    local warning
    warning="$(create_json_event "${category}" "${message}" "${metadata}")"
    WARNINGS+=("${warning}")
...
record_error() {
    ...
    local error
    error="$(create_json_event "${category}" "${message}" "${metadata}")"
    ERRORS+=("${error}")
...
generate_markdown_summary() {
    ...
    local template_file
    template_file="$(dirname "${BASH_SOURCE[0]}")/../templates/summary_template.md"
