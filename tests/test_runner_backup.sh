...
# Test summarization template functionality
test_summarization_template() {
    echo "Testing summarization template functionality..."

    # Source the summarization test script and run its tests
    # shellcheck source=./summarization_test.sh
    source "${SCRIPT_DIR}/summarization_test.sh"

    if run_tests >/dev/null 2>&1; then
        echo -e "${GREEN}✓${NC} summarization_template_tests: PASSED"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}✗${NC} summarization_template_tests: FAILED"
        FAILED_TESTS+=("summarization_template_tests")
    fi
    ((TESTS_RUN++))
}
...
run_tests() {
    ...
    cleanup_tests    # Print test results
    echo "=================================================="
    echo "Test Results:"
